...
function drawAnnotation(
  ...
  if (annotation.kind === "highlight") {
    page.drawRectangle({
      x: point.x,
      y: point.y - 18,
      width: Math.min(width * 0.42, width - point.x - 12),
      height: 24,
      color: rgb(1, 0.88, 0.28),
      opacity: 0.48,
    });
    return;
  }
  if (annotation.kind === "draw") {
    ...
    for (let index = 1; index < points.length; index += 1) {
      const previous = pagePoint(
        { ...annotation, ...points[index - 1] },
        width,
        height,
      );
      const current = pagePoint(
  ...
  if (annotation.kind === "delete") {
    page.drawRectangle({
      x: point.x - 3,
      y: point.y - 20,
      width: Math.min(width * 0.52, width - point.x - 12),
      height: 27,
      color: rgb(1, 1, 1),
