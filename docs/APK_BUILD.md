...
The artifact is an **installable debug APK** signed with Android’s debug key. The workflow verifies that the APK contains both `armeabi-v7a` (32-bit ARM) and `arm64-v8a` (64-bit ARM) native libraries before it uploads the artifact. After a successful workflow run, open the run summary, download `PDF-Studio-debug-APK`, unzip the downloaded artifact, and install `app-debug.apk` on an Android device. Android may ask for permission to install apps from the file manager or browser used for the download.
